...
public class CDRAnalysis {
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(CDRAnalysis.class);
		...
		public void map(Object key, Text value, Mapper<Object, Text, Text, LongWritable>.Context context) throws IOException, InterruptedException {
			...
			if (parts[CDRConstants.STDFlag].equalsIgnoreCase("1")) {
				...
				String callStartTime = parts[CDRConstants.callStartTime];
				 try {
				 Date date1=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(callStartTime);  
				 Date date2=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(callEndTime);
